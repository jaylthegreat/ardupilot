...
// get_surface_tracking_climb_rate_terrfollow- hold vehicle at the desired distance above the ground
// this version uses an inputted value for rangefinder reading so that it can be handled elsewhere
//      returns climb rate (in cm/s) which should be passed to the position controller
float Sub::get_surface_tracking_climb_rate_terrfollow(int16_t target_rate, float current_alt_target,
    float dt, uint16_t rngfnd_alt)
{
    ...
    target_rangefinder_alt = constrain_float(target_rangefinder_alt, 
        rngfnd_alt-pos_control.get_leash_down_z(),
        rngfnd_alt+pos_control.get_leash_up_z()
    );

    // calc desired velocity correction from target rangefinder alt vs actual rangefinder alt (remove the error already passed to Altitude controller to avoid oscillations)
    distance_error = (target_rangefinder_alt - rngfnd_alt) - (current_alt_target - current_alt);
    velocity_correction = distance_error * g.rangefinder_gain;
    velocity_correction = constrain_float(velocity_correction, -THR_SURFACE_TRACKING_VELZ_MAX,
